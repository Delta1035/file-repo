...
- justify-content 属性定义了项目在主轴上的对齐方式。justify 使...对齐, 使content对齐
- align-items 属性定义项目在交叉轴上如何对齐。align 使...排成一条线, 使item对齐
- align-content 属性定义了多根轴线的对齐方式。如果项目只有一根轴线，该属性不起作用。
  - stretch; item的高度将会铺满整个容器, 如果有多行, 高度将会每行均分
  - 当设置flex-wrap:wrap;时, 单行放不下的item将会换行, 从而形成多主轴

以下 6 个属性设置在**项目上**：

- order 属性定义项目的排列顺序。数值越小，排列越靠前，默认为 0。
- flex-grow 属性定义项目的放大比例，默认为 0，即如果<span style="color:red">当前主轴</span>存在剩余空间，也不放大。
  - 分配当前主轴的剩余空间, 即使其他主轴还有剩余空间也不会分配其他主轴
  - 如果只有一个item设置flex:1;  那么剩余空间都会分配给该item
  - 如果多个iten分配了flex, 将会按照flex/ flex总和分配
- flex-shrink 属性定义了项目的缩小比例，默认为 1，即如果空间不足，该项目将缩小。
  - 默认情况下, 空间不足时 item的空间会缩小,
  - 设置为0, 则该元素不会缩小
- flex-basis 属性定义了在分配多余空间之前，项目占据的主轴空间。浏览器根据这个属性，计算主轴是否有多余空间。它的默认值为 auto，即项目的本来大小。
  - flex-basis: auto(默认) | content(内容大小) | 300px(指定的长度,会覆盖width)
- flex 属性是 flex-grow，flex-shrink 和 flex-basis 的简写，默认值为 0 1 auto。
  - flex:1; 一般表示 flex-grow:1; 占据剩余空间
