...
![](https://static.zhufengpeixun.com/dui_de_nei_cun_fen_pei_1639157736483.jpg)
...
#### 1.2.1 堆空间分类 [#](#t3121-堆空间分类)

##### 1.2.1.1 新生代(new space) [#](#t41211-新生代new-space)

* 新生代内存用于存放一些生命周期比较短的对象数据

##### 1.2.1.2 老生代(old space) [#](#t51212-老生代old-space)
...
##### 1.2.1.3 Code space [#](#t61213-code-space)

* 用于存放JIT已编译的代码
* 唯一拥有执行权限的内存

##### 1.2.1.4 Large object space [#](#t71214-large-object-space)

* 为了避免大对象的拷贝，使用该空间专门存储大对象
* GC 不会回收这部分内存

##### 1.2.1.5 Map space [#](#t81215-map-space)

* 存放对象的Map信息，即隐藏类
* 隐藏类是为了提升对象属性的访问速度的
* V8 会为每个对象创建一个隐藏类，记录了对象的属性布局，包括所有的属性和偏移量

#### 1.2.2 什么是垃圾 [#](#t9122-什么是垃圾)
...
#### 1.2.3 新生代的垃圾回收 [#](#t10123-新生代的垃圾回收)
...
#### 1.2.4 老生代的垃圾回收 [#](#t11124-老生代的垃圾回收)
...
##### 1.2.4.1 Mark-Sweep(标记清除) [#](#t121241-mark-sweep标记清除)
...
##### 1.2.4.2 Mark-Compact（标记整理） [#](#t131242-mark-compact标记整理)
...
##### 1.2.5.1 Parallel(并行执行) [#](#t151251-parallel并行执行)
...
##### 1.2.5.3 Write-barrier(写屏障) [#](#t171253-write-barrier写屏障)
...
##### 1.2.5.4 Lazy Sweeping(惰性清理) [#](#t181254-lazy-sweeping惰性清理)

* 当增量标记完成后，如果内存够用，先不清理，等JS代码执行完慢慢清理

##### 1.2.5.5 concurrent(并发回收) [#](#t191255-concurrent并发回收)
...
##### 1.2.5.6 并发(concurrent)和并行(parallel) [#](#t201256-并发concurrent和并行parallel)
...
### 2.1 什么是内存泄露 [#](#t2221-什么是内存泄露)

* 那当不再用到的对象内存没有及时被回收时，我们叫它内存泄漏

### 2.2 不合理的闭包 [#](#t2322-不合理的闭包)
...
**  3.2 定位内存泄漏 [#](#t3232-定位内存泄漏)**

#### 3.2.1 录制监控 [#](#t33321-录制监控)

* 刷新录制页面加载
* 监控堆、文档 、节点、监听器、CPU
* 手工GC

#### 3.2.2 内存快照 [#](#t34322-内存快照)

字段 含义 摘要 按构造函数进行分组，捕获对象和其使用内存的情况 对比 对比某个操作前后的内存快照区别 控制 查看堆的具体内容，可以用来查看对象结构 统计信息 统计视图

#### 3.2.2.1 摘要 [#](#t353221-摘要)
...
**  4.3 尽量创建对象一次搞定 [#](#t4043-尽量创建对象一次搞定)**
...
**  4.4 尽量保持参数结构稳定 [#](#t4144-尽量保持参数结构稳定)**
