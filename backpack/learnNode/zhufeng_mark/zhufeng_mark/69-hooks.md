...
date: null
publisher: 珠峰架构师成长计划
stats: paragraph=67 sentences=118, words=1548
---
...
![](http://img.zhufengpeixun.cn/useLayoutEffects.jpg)
...
function Counter(){
    ...
    return (
        <>
            <p>{name}:{number}p>
             <button onClick={()=>setName('架构')}>修改名称button>
            <button onClick={()=>setNumber(number+1)}>+button>
        </>
    )
}
...
### 6.2 实现useEffect [#](#t1362-实现useeffect)
...
## 7. useLayoutEffect [#](#t147-uselayouteffect)
